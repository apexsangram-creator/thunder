(function () {
  ...
  // This script is served only with our own live demos — it is never part of
  // the template package a customer downloads. Finding it on any other host
  // means the page was copied from a demo, so those pages get a licence
  // notice instead of the theme panel. Local addresses stay exempt so that
  // offline testing is not flagged.
  var LICENSED_HOST = /(^|\.)awaikenthemes\.com$/i;
  var LOCAL_HOST =
    /^(localhost|127\.0\.0\.1|\[::1\])$|\.(local|test|localhost)$/i;

  var hostname = window.location.hostname;

  if (!LICENSED_HOST.test(hostname)) {
    if (!LOCAL_HOST.test(hostname) && window.location.protocol !== "file:") {
      onReady(showLicenseNotice);
    }
    return;
  }

  // Past this point the host is ours. Analytics loads here — before the slug
  // check below, so pages without a theme slug still report — which keeps
  // scraped copies out of the container entirely.
  ...
  function getLinkSource() {
    ...
    var source = /(^|\.)templatemonster\.com$/i.test(referrerHost())
      ? "tm"
      : "tf";
  ...
  function isThemeForestReferred() {
    ...
    if (
      params &&
      (params.get(TF_MARKER) === "tf" ||
        params.get("utm_source") === "themeforest")
    ) {
  ...
  function buildPanel(themes) {
    ...
    // A WordPress demo needs its own landing post — without it there is
    // nothing to link to. An HTML demo does not: it renders only the
    // cross-sell popup, which is driven by the WordPress landing post
    // (themes[baseSlug]). Older HTML templates never got a "<slug>-html"
    // post, so requiring one here silenced the popup on all of them.
    if (!isHtml && !themes[landingKey]) {
      return;
    }
    ...
    // Nothing to show at all — no purchase link, no docs, no cross-sell.
    if (!buyLink && !docLink && !wpDemo) {
      return;
    }
    ...
    // Only when the landing post actually exists, so these never point at a 404.
    if (themes[landingKey]) {
    ...
    if (wpDemo) {
      ...
      popup
        .querySelector(".html-popup-close")
  ...
  /* ---------------------------------------------------------------------
   * Licence notice, shown when this script runs off a host that is not ours
   * ------------------------------------------------------------------ */

  // 	function showLicenseNotice() {
  // 		if (!document.body || document.querySelector('.ath-license-notice')) {
  // 			return;
  // 		}

  // 		var link = 'https://themeforest.net/user/awaiken/portfolio';

  // 		try {
  // 			var url = new URL(link);
  // 			url.searchParams.set('utm_source', 'unlicensed-copy');
  // 			url.searchParams.set('utm_medium', 'license-notice');
  // 			url.searchParams.set('utm_campaign', hostname);
  // 			link = url.toString();
  // 		} catch (e) {}

  // 		var notice = `
  // 		<div class="ath-license-notice">
  // 			<div class="ath-license-notice-inner">
  // 				<p class="ath-license-notice-title">Unlicensed Template</p>
  // 				<p class="ath-license-notice-text">This website appears to be using a copy of an Awaiken Themes template taken from our demo site without a valid license.

  // The original template is officially available from Awaiken Themes. Please purchase a valid license for the original template to use it legally.</p>
  // 				<a class="ath-license-notice-btn" href="${link}" target="_blank" rel="noopener">Get The Original Template</a>
  // 			</div>
  // 		</div>
  // 		<style type="text/css">
  // 			.ath-license-notice {
  ...
  // 				border-top: 3px solid #d2e761 !important;
  ...
  // 			}
  // 			.ath-license-notice-inner {
  // 				max-width: 720px !important;
  // 				margin: 0 auto !important;
  // 			}
  // 			.ath-license-notice-title {
  // 				margin: 0 0 8px !important;
  // 				padding: 0 !important;
  // 				color: #d2e761 !important;
  // 				font-size: 17px !important;
  // 				font-weight: 700 !important;
  // 				letter-spacing: 0.05em !important;
  // 				text-transform: uppercase !important;
  // 			}
  // 			.ath-license-notice-text {
  // 				margin: 0 0 18px !important;
  // 				padding: 0 !important;
  // 				color: #ffffffcc !important;
  // 				font-size: 15px !important;
  // 			}
  // 			.ath-license-notice-btn {
  // 				display: inline-block !important;
  // 				background: #d2e761 !important;
  ...
  function loadGoogleTagManager() {
    if (document.getElementById("gtm-script-dynamic")) {
      return;
    }
